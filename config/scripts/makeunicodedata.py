...
CATEGORY_NAMES = [ "Cc", "Cf", "Cn", "Co", "Cs", "Ll", "Lm", "Lo", "Lt",
    "Lu", "Mc", "Me", "Mn", "Nd", "Nl", "No", "Pc", "Pd", "Pe", "Pf",
...
SCRIPT_NAMES = [ "Common", "Latin", "Greek", "Cyrillic", "Armenian",
    ...
    "Hangul", "Ethiopic", "Cherokee", "Canadian_Aboriginal", "Ogham",
    "Runic", "Khmer", "Mongolian", "Hiragana", "Katakana", "Bopomofo",
    "Han", "Yi", "Old_Italic", "Gothic", "Deseret", "Inherited", "Tagalog",
    "Hanunoo", "Buhid", "Tagbanwa", "Limbu", "Tai_Le", "Linear_B",
    "Ugaritic", "Shavian", "Osmanya", "Cypriot", "Braille", "Buginese",
    "Coptic", "New_Tai_Lue", "Glagolitic", "Tifinagh", "Syloti_Nagri",
    "Old_Persian", "Kharoshthi", "Balinese", "Cuneiform", "Phoenician",
    "Phags_Pa", "Nko", "Sundanese", "Lepcha", "Ol_Chiki", "Vai",
    "Saurashtra", "Kayah_Li", "Rejang", "Lycian", "Carian", "Lydian",
    "Cham", "Tai_Tham", "Tai_Viet", "Avestan", "Egyptian_Hieroglyphs",
...
def makeunicodedata(unicode, trace):
    ...
    print(file=fp)
    print("#ifndef INTERNAL_UNICODEDATA_DB_H", file=fp)
    print("#define INTERNAL_UNICODEDATA_DB_H 1", file=fp)
    print(file=fp)
    ...
    print("static const char *category_names[] = {", file=fp)
    for name in CATEGORY_NAMES:
        print("    \"%s\"," % name, file=fp)
    print("    NULL", file=fp)
    print("};", file=fp)

    print("static const char *bidirectional_names[] = {", file=fp)
    for name in BIDIRECTIONAL_NAMES:
        print("    \"%s\"," % name, file=fp)
    print("    NULL", file=fp)
    print("};", file=fp)

    print("static const char *east_asian_width_names[] = {", file=fp)
    ...
    for name in EASTASIANWIDTH_NAMES:
        print("#define UCDN_EAST_ASIAN_%s %d" % (name.upper(), EASTASIANWIDTH_NAMES.index(name)), file=fp)
    print(file=fp)
    ...
    for name in BIDIRECTIONAL_NAMES:
        print("#define UCDN_BIDI_CLASS_%s %d" % (name.upper(), BIDIRECTIONAL_NAMES.index(name)), file=fp)
    print(file=fp)
...
class UnicodeData:
    ...
    def __init__(self, version,
        ...
        with open_data(BIDI_MIRRORING, version) as file:
            for s in file:
                s = s.partition('#')[0]
                s = [i.strip() for i in s.split(';')]
                if len(s) < 2:
